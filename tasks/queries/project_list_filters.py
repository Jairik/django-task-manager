"""Apply home-page project list filters and sort to a Project queryset.

Toolbar URL params are parsed in ``tasks.project_list_ui``; this module maps that
...
from tasks.project_list_ui import DEFAULT_DUE_FILTER, DEFAULT_SORT_DIR, DEFAULT_SORT_FIELD
...
if TYPE_CHECKING:
    from tasks.project_list_ui import ProjectListUIState

# Sort fields supported by the toolbar.
_VALID_SORT_FIELDS = frozenset({"updated", "due_date", "priority", "created", "name"})
_VALID_SORT_DIRS = frozenset({"asc", "desc"})


@dataclass(frozen=True)
class ProjectListFilters:
    """Filter and sort options for the home project grid."""
...
def project_list_filters_from_ui_state(ui_state: "ProjectListUIState") -> ProjectListFilters:
...
def apply_project_list_filters(
    ...
    *,
    ...
    """Narrow a project queryset by toolbar priority and due-date filters."""
    if today is None:
...
def apply_project_list_sort(
    queryset: QuerySet[Project],
    filters: ProjectListFilters,
) -> QuerySet[Project]:
    """Order a project queryset by the toolbar sort field and direction."""
